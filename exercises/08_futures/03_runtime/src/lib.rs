    ...
    T: Display + Send + Sync + 'static,
{
    ...
    tokio::spawn(serve_listener(first, reply.clone()));
    tokio::spawn(serve_listener(second, reply.clone()));
}


pub async fn serve_listener<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
